...
register INODE *ip;
{
	...
	case IFPIPE:
		...
		ip->i_a.i_pnc = dip->di_a.di_pnc;
		canint(ip->i_a.i_pnc);
		ip->i_a.i_prx = dip->di_a.di_prx;
		canint(ip->i_a.i_prx);
		ip->i_a.i_pwx = dip->di_a.di_pwx;
		canint(ip->i_a.i_pwx);
		break;
	default:
...
register INODE *ip;
{
	...
	case IFPIPE:
		...
		dip->di_a.di_pnc = ip->i_a.i_pnc;
		canshort(dip->di_a.di_pnc);
		dip->di_a.di_prx = ip->i_a.i_prx;
		canshort(dip->di_a.di_prx);
		dip->di_a.di_pwx = ip->i_a.i_pwx;
		canshort(dip->di_a.di_pwx);
		break;
	default:
