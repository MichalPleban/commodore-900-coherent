...
readhdr()
{
	...
	while ((header = readblk()) != NULL) {
		if (header->th_name[0] == '\0')
			...
			/*
			 * Blanking th_check in place corrupted records
			 * that ungetblk pushed back (tar u re-read them
			 * with a spaces checksum and re-appended
			 * unchanged files); restore the field after
			 * summing.
			 */
