 ...
 *  Sep 12, 1984 (Charles Fiterman)
 *	    Added documentation on kermit protocol to end of kermit.c
 *  Sep 21, 1984 (Charles Fiterman)
 *	    Added -m switch for MacIntosh mode to communicate with
 *	    MacKermit. This places 2 EOL's at the end of a line.
 *  Nov 13, 1984 (Charles Fiterman)
 *	    Added -x switch to leave slashes and xmit directory
 *	    structure. (used x because -s and -d were taken).
 *  Jan 10, 1985 (Charles Fiterman)
 *	    Added -a switch causes filenames to be seen in pairs
 *	    The first is the name on the send side, the second
 *	    Is the name on the recieve sidee.
 */
...
#if COHERENT
#define DEFLINE	    "/dev/tty38"    /* default tty comm line */
#define DEFSPEED    2400	  /* default tty speed */ 	
#endif
...
int	size,		    /* Size of present data */
	...
	mflg,		    /* Flag for MacKermit mode */
	xflg,		    /* flag for xmit directory structure */
	aflg,		    /* flag for -a switch filenames in pairs */
...
/*  Initialize these values and hope the first packet will get across OK */
    ...
    speed = hflg = cflg = sflg = mflg = 0;     /* Turn off all parse flags */
    rflg = tflg = xflg = aflg = 0;
	...
	switch (*cp++)
	{
	    ...
	    case 'm': mflg++; break;	/* M = MacKermit mode */
	    case 'h': hflg++; break;	/* H = Host (server) mode */
	    case 't': tflg++; break;	/* T = Tymnet mode */
	    case 'x': xflg++; break;	/* X = Directory mode */
	    case 'a': aflg++; break;	/* A = Filenames in pairs */
...
/* All set up, now execute the command that was given. */
	...
	if (argc--) {
		...
		if (aflg && argc--)
			filnamo = *argv++;	/* Get second name of pair */
		else
			filnamo = filnam;
	} else
	{   if (remote)
...
connect()
{
    ...
    char inbuff[128];			/* remote data input buffer */
    register char *p; 	
    register int count;
    int rcount;
    char bel = '\07',
...
int num, len;
{
    ...
    *bufp++ = tochar(chksum);		/* Put it in the packet */
    if ( mflg ) *bufp++ = eol;		/* MacKermit needs this */
...
/* Table 1: The six fields in a packet of information in
	    Kermit protocol

MARK	Start of packet character, normally SOH ^A

LEN	Number of ASCII characters including prefixing and
	checksum in the rest of the packet.

SEQ	The packet sequence number mod 64.

TYPE	The packet type, one of the following.
	D  Data
	Y  ACK
	N  NAK
	S  Send Initiate followed by the following data
		MAXL  max packet length you want me to send
		TIME  Time in seconds I want you to wait for me
		      before timing out.
		NPAD  Number of padding characters before each
		      before each message.
		PADC  The control character I need for padding
		      modified by ^ 64. Ignored if zero.
		EOL   The character I need to terminate an incoming
		      packet.
		QCTL  The printable ASCII character I will use to
		      Quote control characters, normally #.
		QBIN  The printable ASCII character I will use to
		      Quote characters with hi order bit on.
		CHKT  Check type.
			1 single character checksum.
			2 two character checksum.
			3 three character CRC-CCITT.
		REPT  Prefix character for repeated characters,
		      Blank turns this off.
		CPAS  An extendable bit mask encoded to indicate
		      whether certan advanced capabilities such
		      as file-attribute packets are supported.
		Reserved for future expansion.

	R  Recieve-Initiate (answer to send init)
	...
	G  Generic command.
		L  Logout, bye.
		F  Finish don't log out.
		D  Directory query (followed by file spec)
		U  Disk usage query
		E  Erase followed by file spec.
		T  Type followed by file spec.
		Q  Query server status.
	C  Host command. Data to be executed by host system.
	X  Display text header.

DATA	The contents of the packet. All characters will be display.
	unprintable characters prefixed by QCTL, characters with
	8 bit set prefixed by QBIN. Repeated characters prefixed
	by REPT and a count.

CHECK	The block check sequence.
*/
