...
/*
 * Coherent 0.7.3 port.  This libc predates the ANSI <string.h> names:
 *   strrchr()  is spelled rindex()  (the V7/BSD name libc actually ships)
 *   memset()   is absent            -> a local definition is provided below
 * Neither change touches the compression algorithm itself.
 */
...
/*
 * Coherent 0.7.3 libc has no memset(); provide one.  Only the VIRTUAL block
 * needs it, to zero a block that was paged in but had never been written.
 */
char *
memset(s, c, n)
char *s;
