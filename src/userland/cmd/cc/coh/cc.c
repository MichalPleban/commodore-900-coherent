...
resolve()
{
	...
	/*
	 * -E is the preprocessor alone.  This was under TEMPBUF, so a
	 * build without it ran cc0 on the preprocessed text as well.
	 */
...
compile(argc, argv) int argc; char *argv[];
{
	...
	for (i=1; i<argc; ++i) {
		...
		setfiles(c, p1);
		if (c == CCARG) {
			if (runpp(argc, argv, vstr, pass[CPP].p_ofn)) {
				if (Aflag) {
					++err; --i;
				} else
					++nload;
			...
			if (runpp(argc, argv, vstr2, pass[CC0].p_ofn)) {
				if (Aflag) {
					++err; --i;
				} else
					++nload;
...
/*
 * The preprocessor is its own pass (/lib/cpp: cc0 built without the
 * parser).  It takes the same variant word as cc0 with VCPP set.  The word
 * is 12 hex digits, byte i at digits 2i and 2i+1, bit x of byte x/VGRANU.
 */
char *
cppvariant(var) char *var;
{
	static char v[2*VMAXIM/VGRANU + 1];
	...
	strcpy(v, var);
	p = v + 2 * (VCPP / VGRANU);
	b = (index(digit, p[0]) - digit) << 4 | (index(digit, p[1]) - digit);
	b |= 1 << (VCPP % VGRANU);
	p[0] = digit[b >> 4];
	p[1] = digit[b & 15];
	return v;
...
runpp(argc, argv, var, out) int argc; register char *argv[]; char var[]; char *out;
{
	register char **cp;
	register int i;
	char *p1;
