...
/*
 * `void *' is `char *' here.  A pointer to void is an object pointer
 * with no type of its own, and this front end (and its headers, which
 * spell NULL as a char pointer) has only char * for that job.  The
 * declarator chain runs outermost first, so the LAST dim is the one
 * next to the base type: a pointer there means pointer, pointer to
 * pointer, array of pointer or function returning pointer to void --
 * all of them char.  A function returning void (last dim D_FUNC) or a
 * plain void stays void.
 */
