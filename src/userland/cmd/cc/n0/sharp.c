...
define()
{
	...
	if (dputp > dbuf && dputp[-1] == ' ')
		--dputp;
	dputc(0);
	if (sp != NULL && sp->s_value <= XUSERA && sp->s_narg == narg
	 && bodyeq(dbuf, sp->s_body)) {
		/* the same definition again: silent */
#if ANSI_STUPID
		for (c = 0; c < narg; c += 1)
		...
		/*
		 * A different definition.  This was an error; K&R cpp and
		 * the compiler this tree is otherwise built with take the new
		 * definition with a warning, and the code relies on it (the
		 * kernel is compiled -DK1 for a driver whose keyboard table
		 * header defines K1 as a scan code; two headers spell BSIZE
		 * with different blanks).  The new SYM goes in front of the
		 * old one, so the lookup finds it first.
		 */
		if (sp != NULL)
			cwarn("\"%s\" redefined", tp->t_id);
		if (narg > NARGS) {
...
/*
 * Are two macro bodies the same definition?  Blanks between tokens are
 * separation, not content: a run of them on both sides is one, a run on
 * one side only is a difference, and trailing ones are nothing.
 */
static
bodyeq(a, b) register char *a, *b;
{
	register int wa, wb;
...
#endif	/* !CPPONLY: n0/cppeval.c has the standalone one */
...
#ifdef	CPPONLY
		/*
		 * Not this program's to judge: hand the line on to the
		 * compiler (no newline -- the directive's own follows).
		 */
		fprintf(ofp, "#pragma %s", p);
#else
		if (pragma(p) == 0)
...
line:
		...
		getnum(c, 0);
		/*
		 * #line N numbers the NEXT line N.  The directive's own
		 * newline is still in the stream and will count one, so N-1
		 * here.  (It was N, which put every #line -- a user's, or
		 * cpp's own output read back by cc0 -- one line late.)
		 */
		line = ival - 1;
		c = getfname();
		dputp = dbuf;
