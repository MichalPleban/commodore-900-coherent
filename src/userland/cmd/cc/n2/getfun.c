 ...
 * every function's records stayed allocated until exit and a large
 * file held all of its instructions at once -- more than a 64 K data
 * segment on the C900.  Nothing points into the list once genfunc()
 * has emitted it: fixups name symbols, not instructions.
 */
freefunc()
{
	register INS	*ip, *np;
