...
LIBL_OBJ  := $(patsubst $(SRC)/%.c,$(OBJ)/%.o,$(libl_c))
...
# libl: the lex(1) run time (lexlib.c, plus default yywrap/main members)
$(LIBDIR)/libl.a: $(LIBL_OBJ)
	$(ar-sorted)
...
$(BINDIR)/db $(BINDIR)/ed $(BINDIR)/egrep $(BINDIR)/enroll $(BINDIR)/expr $(BINDIR)/find \
...
# The C compiler: /lib/cpp cc0 cc1 cc2 cc3 and /bin/cc.  Mark Williams C
# 4.2.12 with a Z8001 back end (kdedon/commodore-900-toolchain, patched: see
# CC_BUGS.md and src/userland/cmd/cc/README.md); it replaced the 1985
# binaries that shipped from src/dist.  One source, two front-end programs:
# cc0.c/get.c/cpp.c compile once with -DNOCPP (the parser, /lib/cc0) and once
# with -DCPPONLY (the preprocessor, /lib/cpp) into a separate object tree
# ($(CCOBJ)/pp).  cc0/cc1/cc2 link -L with their own heap over four 60 K
# arenas in modules of their own (common/z8001/bigheap.c, arena0..3.c): a
# kernel-sized file needs more than one 64 K data segment leaves.  Debug
# tables and the listing are stubbed (nodbg0/nodbg2).  n1/tables/macros.c and
# patern.c are GENERATED by tabgen from the .t grammar and committed (the
# README says how to regenerate them).
CCSRC := $(CMDS)/cc
CCOBJ := $(OBJ)/userland/cmd/cc
CCINC := -I$(CCSRC)/n1/z8001 -I$(CCSRC)/n0/z8001 -I$(CCSRC)/h/z8001 \
	-I$(CCSRC)/generated -I$(CCSRC)/n0 -I$(CCSRC)/n1 -I$(CCSRC)/h \
	-I$(CCSRC)/common -I$(CCSRC)/n3 -I$(CCSRC)/n3/z8001 -I$(CCSRC)/h/compat
cc_common := $(addprefix $(CCOBJ)/common/,bget.o bput.o dget.o diag.o dput.o \
	getvar.o iget.o iput.o lget.o lput.o makvar.o milnam.o mionam.o newlab.o \
	newseg.o nput.o segnam.o sget.o sput.o talloc.o tcpy.o unbget.o)
cc_heap := $(addprefix $(CCOBJ)/common/z8001/,bigheap.o arena0.o arena1.o arena2.o arena3.o)
cc_n0dual := $(addprefix $(CCOBJ)/n0/,cc0.o get.o cpp.o)
cc_n0 := $(cc_n0dual) $(addprefix $(CCOBJ)/n0/,cc0key.o cc0sym.o ddecl.o \
	dope.o double.o etc.o expr.o fold.o gcandt.o gdecl.o init.o lex.o \
	locals.o size.o stat.o split/nocpp.o split/nodbg0.o z8001/bind.o)
cc_pp := $(addprefix $(CCOBJ)/pp/,cc0.o get.o cpp.o sharp.o expand.o \
	cppeval.o ppstub.o) $(addprefix $(CCOBJ)/n0/,lex.o cc0key.o cc0sym.o etc.o)
cc_n1 := $(addprefix $(CCOBJ)/n1/,cc1.o cc1sym.o code.o mtree0.o mtree1.o \
	mtree3.o node.o out.o pool.o reg0.o sel0.o sel1.o snap0.o tree0.o tree1.o \
	tables/macros.o tables/patern.o) $(addprefix $(CCOBJ)/n1/z8001/,altemp.o \
	amd.o fixtop.o gen1.o gen2.o mtree2.o mtree4.o outmch.o reg1.o sel2.o \
	snap1.o table0.o table1.o)
cc_n2 := $(addprefix $(CCOBJ)/n2/,cc2.o cc2sym.o emit0.o getfun.o optim.o) \
	$(addprefix $(CCOBJ)/n2/z8001/,afield.o emit1.o getcod.o nodbg2.o optab.o \
	outcoh.o peep.o)
cc_n3 := $(addprefix $(CCOBJ)/n3/,cc3.o dbgt3.o itree.o) \
	$(addprefix $(CCOBJ)/n3/z8001/,icode.o igen.o) \
	$(addprefix $(CCOBJ)/common/z8001/,mdlnam.o mdonam.o regnam.o tyname.o)
cc_drv := $(CCOBJ)/coh/cc.o $(CCOBJ)/common/makvar.o
$(cc_common) $(cc_heap) $(cc_n0) $(cc_n1) $(cc_n2) $(cc_n3): CFLAGS += $(CCINC)
$(cc_n0dual): CFLAGS += -DNOCPP
# The driver: 4.2 spellings of access(2)'s modes, a version string, and the
# i386/i8086 floating-point variant names its option table lists mapped onto
# one unused machine slot, all supplied with -D so the source is not forked.
# Its static getpass() is renamed clear of getpass(3).
$(CCOBJ)/coh/cc.o: CFLAGS += -I$(CCSRC)/h -I$(CCSRC)/h/z8001 -I$(CCSRC)/generated -I$(CCSRC)/h/compat \
	-DCOHERENT=1 -DX_OK=AEXEC -DR_OK=AREAD -DVERSMWC=\"4.2-z8001\" \
	-DV8087=VMBASE+13 -DVNDP=VMBASE+13 -DVOMF=VMBASE+13 -DV80186=VMBASE+13 \
	-DV80287=VMBASE+13 -DVEMU87=VMBASE+13 -Dgetpass=ccgetpass -DL_tmpnam=64
$(CCOBJ)/pp/%.o: $(CCSRC)/n0/%.c $(HDRS)
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $(CCINC) -DCPPONLY -c $< -o $@
$(CCOBJ)/pp/%.o: $(CCSRC)/n0/split/%.c $(HDRS)
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $(CCINC) -DCPPONLY -c $< -o $@
$(LIBDIR)/cpp: $(cc_pp) $(cc_common) $(CRT) $(LIBC) $(LIBC_SL)
	$(call link,$(cc_pp) $(cc_common))
$(LIBDIR)/cc0 $(LIBDIR)/cc1 $(LIBDIR)/cc2: LDNFLAGS := -L
$(LIBDIR)/cc0: $(cc_n0) $(cc_common) $(cc_heap) $(CRT) $(LIBC) $(LIBC_SL)
	$(call link,$(cc_n0) $(cc_common) $(cc_heap))
$(LIBDIR)/cc1: $(cc_n1) $(cc_common) $(cc_heap) $(CRT) $(LIBC) $(LIBC_SL)
	$(call link,$(cc_n1) $(cc_common) $(cc_heap))
$(LIBDIR)/cc2: $(cc_n2) $(cc_common) $(cc_heap) $(CRT) $(LIBC) $(LIBC_SL)
	$(call link,$(cc_n2) $(cc_common) $(cc_heap))
$(LIBDIR)/cc3: $(cc_n3) $(cc_common) $(CRT) $(LIBC) $(LIBC_SL)
	$(call link,$(cc_n3) $(cc_common))
$(BINDIR)/cc: $(cc_drv) $(CRT) $(LIBC) $(LIBC_SL)
	$(call link,$(cc_drv))
CC_TARGETS := $(LIBDIR)/cpp $(LIBDIR)/cc0 $(LIBDIR)/cc1 $(LIBDIR)/cc2 $(LIBDIR)/cc3 $(BINDIR)/cc
cc: $(CC_TARGETS)
...
# db: the assembler-level symbolic debugger, reconstructed from the shipped
# binary (_disasm/db.asm; the original is kept as _disasm/db.orig).
db_obj := $(addprefix $(OBJ)/userland/cmd/db/,main.o run.o cmd.o expr.o sym.o dis.o data.o)
$(db_obj): CFLAGS += -I$(CMDS)/db
$(BINDIR)/db: $(db_obj) $(CRT) $(LIBC)
	$(call link,$(db_obj))
...
# to /lib by the dist target below (it used to be a second copy under
# src/dist) -- it is NOT linked into the yacc binary (the command's own
# makefile links only y?.o).  Distinct from the *host* coherent-yacc ($(YACC))
...
CMD_TARGETS := $(BIN_TARGETS) $(ETC_TARGETS) $(KERN_ETC_TARGETS) $(GAMES_TARGETS) \
	...
	$(BINDIR)/db $(BINDIR)/ed $(BINDIR)/egrep \
...
#   bin/       man script, true/false  (the C compiler -- /bin/cc and the
#              /lib passes cpp,cc0..cc3 -- is BUILT from src/userland/cmd/cc
#              since Sept 2026; the 1985 binaries and the ccx wrapper are gone
#              from here and survive in _disasm/)
#   etc/       cpsys script, boottime marker
#   lib/       yacc skeleton yyparse.c
...
dist: $(DIST_TARGETS) $(ROOT)/lib/yyparse.c

# The yacc parser skeleton HAS a source in this tree, so /lib/yyparse.c is
# installed from there rather than from a src/dist copy that would go stale.
$(ROOT)/lib/yyparse.c: $(CMDS)/yacc/yyparse.c
	@mkdir -p $(dir $@)
	cp $< $@
